...
export class AuditStore extends EventEmitter {
  ...
  /** Rolling in-memory tail for the WebUI /api/executions endpoint. */
  ...
  append(input: BuildRecordInput): AuditRecord {
    ...
    // Push into in-memory rolling tail.
    ...
    // event only fires for records that are durably persisted.
    try {
      this.emit('execution', rec);
    } catch { /* listener errors must not affect the audit path */ }

    return rec;
  }

  /**
   * Read the most-recent N records from the in-memory tail, optionally
   * filtered by profile. Used by the WebUI /api/executions endpoint.
   */
